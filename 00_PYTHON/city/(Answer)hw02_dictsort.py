...
def dict_sort(capital_dict, dictkey=False, index=0, desc=False):
    """
...
def print_list(city_list, index):
    for i, value in enumerate(city_list):
        if index == COUNTRY: # index = 0
            print(f'[{i + 1}] {value[0]:12s}: {value[1][COUNTRY]:15s}')
        elif index == POPULATION: # index = 2
...
def main():
    ...
    while True:
        ...
        if menu == 1:
            for i, key in enumerate(capital_dict):        
                print(f'[{i + 1}] {key}: {capital_dict.get(key)}')
        elif menu == 2:
            ...
            # 국가명 오름차순 출력 
            sorted_list = dict_sort(capital_dict, False, COUNTRY, False)
            print_list(sorted_list, COUNTRY)
        elif menu == 5:
            # 특정 도시의 정보만 출력
            ...
            if continent in ['Asia', 'Europe', 'America']:
